...
/// - 版本号相同 → 无更新；记录不一致时自动修正，避免同版本重新打包造成误报。
///
...
pub fn resolve_update(
    ...
    // 本地实际版本已经与最新 release 相同，视为最新。
    // release 可能因重新打包产生不同 build-id/commit，但这不代表用户
    // 需要重新下载；同时修复可能滞后的本地记录。
    if record_commit != Some(latest.commit.as_str())
        || record_tag.and_then(parse_version_from_tag).as_deref() != Some(latest_version.as_str())
    {
...
mod tests {
    ...
    #[test]
    fn resolve_same_version_repack_heals_without_prompt() {
        // 同版本重新打包不应要求用户反复更新；修复本地 release 记录即可。
        let latest = latest(
    ...
    #[test]
    fn resolve_legacy_same_version_repack_heals() {
        // 老记录无 tag时，同版本重新打包同样只修复记录，不弹更新。
        let latest = latest(
            "dsh-0.1.0-rc.6-31773193667",
            "564019027fd9469991aef6e57bb0a96325491c4e",
        );
