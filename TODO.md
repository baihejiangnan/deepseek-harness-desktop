...
核对日期：2026-09-08；代码基线 v0.0.10 源码（P0/P1/P2 修复见 `a3cf1f8`，文档同步见 `3009d42`）；上述修复自 v0.0.9 起已包含在发行包内，开发分支工作区另有未提交改动。本文只跟踪仍需处理的项目工作，不记录会话流水账。优先级 P0 为数据安全，P1 为优先修复，P2 为后续完善，P3 为可延后清理；状态与验收证据分开。完成后链接提交或验证记录，再移出当前列表。
...
| I18N-01 | P3 / 待决定 | 检查阶段用保守扫描（键名作为字符串字面量出现即算引用，外加 25 个真实动态前缀）在**当时的 925 个键**里找到约 **62 个前后端都没有任何引用**的文案，集中在 `app.*`、`status.*`（`preparing_engine` / `downloading_harness` 等安装阶段）、`plugins.*`、`update.verify_failed`、`errors.*`。它们早于本轮服务商重构，本轮只删掉了服务商自己遗留的 4 个（`providers.imported` / `description` / `remove` / `model`）。**键数此后继续增长：当前实测为 `zh-CN.json` 与 `en-US.json` 各 952 个键、无单侧缺失**（权威值见 [服务商与界面升级方案](docs/PROVIDER_UI_UPGRADE_PLAN.md) 末尾「文案键数」一节），因此这 62 个的归属需按当前键集合重新扫描确认。 | 逐个判定"即将接线"还是"该删"：删则中英两份同步删并复核键集合仍相同；留则在文案旁注明来源与计划。**不得凭扫描结果直接批删**——本仓库有 `t(\`providers.${field}\`)` 这类数据驱动拼 key，判据必须按前缀逐个确认，误删会让界面直接印出原始键名 |
