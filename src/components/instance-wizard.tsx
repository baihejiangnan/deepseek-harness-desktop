...
export default function InstanceWizard(props: { onCancel?: () => void }) {
  ...
  return (
    <div className="mx-auto flex min-h-0 w-full max-w-[1240px] flex-col overflow-hidden px-6 py-4 lg:px-8">
      <PageHeader className="mb-4 shrink-0" title={t('launcher.new_instance')} description={t('launcher.new_instance_description')} />
      <div className="launcher-panel flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border border-[var(--launcher-border)] bg-[var(--launcher-surface)]">
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="grid min-h-full grid-cols-[220px_minmax(0,1fr)] max-md:grid-cols-1">
            <aside className="flex flex-col border-r border-[var(--launcher-border)] bg-[var(--launcher-sidebar)] p-4 max-md:border-b max-md:border-r-0">
              ...
              <div className="mt-3 text-xs leading-5 text-[var(--launcher-muted)]" aria-live="polite">
            ...
            <div className="min-w-0 p-4 lg:px-6">
              <fieldset disabled={submitting} className="m-0 grid min-w-0 grid-cols-2 gap-x-5 gap-y-3 border-0 p-0 max-lg:grid-cols-1">
                ...
                <div className="col-span-2 text-sm max-lg:col-span-1">
                  <span className="mb-2 block text-xs font-medium">{t('providers.choose_new')}</span>
                  {providersLoading
                    ? (
                        <div aria-busy="true" className="flex flex-wrap gap-3">
                          {[0, 1, 2].map(item => <span key={item} className="h-5 w-32 animate-pulse rounded-md bg-[var(--launcher-selected)] motion-reduce:animate-none" />)}
                        </div>
                      )
                    : providerTemplates.length === 0
                      ? <p className="m-0 text-xs leading-5 text-[var(--launcher-muted)]">{t('launcher.wizard_providers_empty')}</p>
                      : (
                  ...
                  <span className="mt-1.5 block text-xs leading-5 text-[var(--launcher-muted)]">{t('launcher.wizard_providers_optional')}</span>
                  {providerError && <p role="alert" className="m-0 mt-1.5 text-xs text-danger">{providerError}</p>}
                </div>
        ...
        <div className="flex shrink-0 flex-wrap justify-end gap-2 border-t border-[var(--launcher-border)] bg-[var(--launcher-surface)] px-4 py-3 lg:px-6">
          ...
          <Button className="h-10 rounded-md bg-[var(--launcher-brand)] px-6 text-[var(--launcher-on-brand)]" isDisabled={submitting || providersLoading || !name.trim() || !dshHome || !profile} onPress={createInstance}>
