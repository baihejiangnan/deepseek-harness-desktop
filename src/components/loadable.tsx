      ...
      <div className="flex w-[min(460px,88vw)] flex-col items-center gap-4 text-center">
        ...
        {showPanel && (
          <div className="flex w-full flex-col gap-4">
            ...
            {hasLogs && (
              <div className="overflow-hidden rounded-lg border border-line bg-log-bg text-left" aria-label={t('ui.install_log')}>
                <div className="flex h-8 items-center justify-between border-b border-line px-3 text-[11px] text-load-muted">
                  <span>{t('ui.install_log')}</span>
                  <button type="button" className="grid size-6 place-items-center rounded text-log-ink hover:bg-panel2" aria-label={t('download.copy_logs')} onClick={() => { void copyLogs() }}>
                    <Copy className="size-3.5" />
                  </button>
                </div>
                <div className="min-h-[80px] max-h-[152px] overflow-y-auto px-3.5 py-2.5 font-mono text-xs leading-[1.7]">
